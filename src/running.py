...
class UnsupervisedRunner(BaseRunner):
    ...
    def masked_snr(
        ...
        for batch in range(batch_count):
            ...
            masked_pred = torch.masked_select(batch_pred, batch_mask).detach().cpu().numpy()

            # Calculate the mean and standard deviation of the data
            mini_batch_snr = []
            channel_start = 0
            for channel_length in per_channel_masks:
                if channel_length == 0:
                    continue
                single_channel_masked_pred = masked_pred[
                    channel_start : channel_start + channel_length
                ]
                mean = np.abs(np.mean(single_channel_masked_pred))
                std = np.std(single_channel_masked_pred)

                # Handle the case where the standard deviation is zero (no noise)
                if std == 0:
                    continue  # Signal to noise ratio is infinite

                # Calculate the signal-to-noise ratio in decibels
                if np.isnan(np.log10(mean / std)):
                    print("")
                    a = 0
                snr = 10 * np.log10(mean / std)
                mini_batch_snr.append(snr)
                channel_start = channel_start + channel_length
            batch_snr.append(np.mean(mini_batch_snr))
        return np.array(batch_snr).mean()
